...
test('arena follow-up shares damage and force and only hits confirmed opponent once',()=>{const ev={prismCast:'c',chaseElapsed:0},me={x:0,y:0,skillEvent:ev},enemy={x:450,y:0,hp:1000,r:18},hits=[];C.markPrismTarget(ev,'c','enemy');const c={window:{EchoesCombat:C},me,enemy,Math,Set,ARENA_W:3600,ARENA_H:2100,clamp:(v,a,b)=>Math.max(a,Math.min(b,v)),beamFx:()=>{},net:()=>{},circleAttack:(...a)=>hits.push(a),pvpSkillAttack:()=>100,riftCutFx:()=>{}};vm.createContext(c);vm.runInContext(source('updatePrismPvpChase'),c);for(let i=0;i<150&&me.skillEvent;i++)c.updatePrismPvpChase(ev,1/60);assert.equal(hits.length,1);assert.equal(hits[0][3],450);assert.equal(hits[0][5].force,180);assert.equal(me.skillEvent,null);});
